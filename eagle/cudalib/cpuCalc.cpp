...
#endif
		{
			probability = calc_read_prob(seq_map, matrix, read_length, seq, seq_length, pos, -1e6);
			...
			for (i = start; i < end; i++) {
				...
				if (i != pos) {
					double val = calc_read_prob(seq_map, matrix, read_length, seq, seq_length, i*read_length, baseline);
